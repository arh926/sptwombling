      ...
      results.grad <- mclapply(parallel.index, function(x){
        ...
        for(i.mcmc in 1:n.mcmc){
          ...
          corr.mat <- st_cor_sepbl(delta = delta, Delta = Delta,
                                   phis = phis[i.mcmc], phit = phit[i.mcmc],
                                   cov.type.s = "matern1",
                                   cov.type.t = "poly")
          ...
          for(i in 1:nrow(grid.points)){
            ...
            HK <- adiag(3 * phis[i.mcmc]^2 * I.d,
                        2 * phit[i.mcmc]^2,
                        6 * phis[i.mcmc]^2 * phit[i.mcmc]^2 * I.d)
        ...
        for(i.mcmc in 1:n.thin.mcmc){
          ...
          for(i in 1:nrow(grid.points)){
            ...
            HK <- adiag(5/3 * phis[i.mcmc]^2 * I.d,
                        25/3 * phis[i.mcmc]^4 * I.tilde,
                        2 * phit[i.mcmc]^2,
                        10/3 * phis[i.mcmc]^2 * phit[i.mcmc]^2 * I.d,
                        50/3 * phis[i.mcmc]^4 * phit[i.mcmc]^2 * I.tilde,
                        24 * phit[i.mcmc]^4,
                        40 * phis[i.mcmc]^2 * phit[i.mcmc]^4 * I.d,
                        200 * phis[i.mcmc]^4 * phit[i.mcmc]^4 * I.tilde)


            # Non-zero off-diagonal entries for HK
            HK[1:2, 13:14] <- HK[13:14, 1:2] <- -10/3 * phis[i.mcmc]^2 * phit[i.mcmc]^2 * I.d
            HK[3:5, 12] <- HK[12, 3:5] <- 10/3 * phis[i.mcmc]^2 * phit[i.mcmc]^2 * I.star
            HK[3:5, 15:17] <-  HK[15:17, 3:5] <- -50/3 * phis[i.mcmc]^4 * phit[i.mcmc]^2 * I.tilde
            HK[6, 9:11] <- HK[9:11, 6] <- -10/3 * phis[i.mcmc]^2 * phit[i.mcmc]^2 * I.star
            HK[12, 15:17] <- HK[15:17, 12] <- -40 * phis[i.mcmc]^2 * phit[i.mcmc]^4 * I.star
        ...
        for(i.mcmc in 1:n.thin.mcmc){
          ...
          for(i in 1:nrow(grid.points)){
            ...
            HK <- adiag(2 * phis[i.mcmc]^2 * I.d,
                        ...
                        2 * phit[i.mcmc]^2,
                        4 * phis[i.mcmc]^2 * phit[i.mcmc]^2 * I.d,
                        8 * phis[i.mcmc]^4 * phit[i.mcmc]^2 * I.tilde,
                        24 * phit[i.mcmc]^4,
                        48 * phis[i.mcmc]^2 * phit[i.mcmc]^4 * I.d,
                        96 * phis[i.mcmc]^4 * phit[i.mcmc]^4 * I.tilde)

            # non-zero entries for HK
            HK[1:2, 13:14] <- HK[13:14, 1:2] <- -4 * phis[i.mcmc]^2 * phit[i.mcmc]^2 * I.d
            HK[3:5, 12] <- HK[12, 3:5] <- 4 * phis[i.mcmc]^2 * phit[i.mcmc]^2 * I.star
            HK[3:5, 15:17] <-  HK[15:17, 3:5] <- -8 * phis[i.mcmc]^4 * phit[i.mcmc]^2 * I.tilde
            HK[6, 9:11] <- HK[9:11, 6] <- -4 * phis[i.mcmc]^2 * phit[i.mcmc]^2 * I.star
            HK[12, 15:17] <- HK[15:17, 12] <- -48 * phis[i.mcmc]^2 * phit[i.mcmc]^4 * I.star
